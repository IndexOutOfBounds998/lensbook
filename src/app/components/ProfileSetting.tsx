        ...
        <div className="p-[15px] w-[300px]">
          <Skeleton loading={loading} avatar paragraph={{ rows: 4 }}>
            <div className="flex items-center mb-[20px]">
              <img
                className="w-[50px] h-[50px] cursor-pointer rounded-3xl mr-[15px]"
                src={profile && formatPicture(profile.picture)}
              />
              <div className="leading-7">
                <div className="hover:text-[#3339] cursor-pointer">
                  <span className="text-[20px] font-bold">
                    {profile && formatNickName(profile.handle)}
                  </span>
                  <i className="iconfont icon-icon-right text-[10px]" />
                </div>
                <p className="truncate w-[200px] text-[#3339] cursor-pointer hover:text-[#000]">
                  {profile?.bio}
                </p>
              </div>
            </div>
            <div className="flex justify-around mb-[30px]">
              {list.map((item, index) => (
                <div
                  key={index}
                  className="text-[#3339] text-center cursor-pointer hover:text-[blueviolet]"
                >
